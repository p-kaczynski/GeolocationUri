...
{
    sealed public class Rule_num:Rule
    {
        private Rule_num(string spelling, List<Rule> rules) :
            base(spelling, rules)
        {
        }
        ...
        public static Rule_num Parse(ParserContext context)
        {
            ...
            if (parsed)
            {
                rule = new Rule_num(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
            }
            else
            {
                context.index = s0;
